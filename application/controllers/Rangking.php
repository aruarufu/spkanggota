...
class Rangking extends MY_Controller
{
    ...
    public function index()
    {
         ...
         */
        $table1 = $this->initialTableSAW($anggota);
    ...
    private function initialTableSAW($anggota)
    {
        $nilai = $this->MNilai->getNilaiUniveristas();

        $dataInput = array();
        $no = 0;
        foreach ($anggota as $item => $itemAnggota) {
            foreach ($nilai as $index => $itemNilai) {
                if ($itemAnggota->kdAnggota == $itemNilai->kdAnggota) {
                    $dataInput[$no]['anggota'] = $itemAnggota->anggota;
